...
const makeCodexSession = (
  ...
  Effect.gen(function* () {
    ...
    const child = yield* Effect.try({
      try: () =>
        // No transport flag: `codex app-server` defaults to `--listen stdio://`.
        // Older builds took an explicit `--stdio`, which current codex rejects
        // outright ("unexpected argument"), killing the spawn.
