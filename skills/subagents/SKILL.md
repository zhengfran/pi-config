...
Requires the Codex CLI to be installed and authenticated. Codex is not
available in the corporate environment; use Kiro there instead and do not
spend a concurrency slot retrying Codex.

## Kiro CLI Harness

**Harness:** `kiro`
**Prompt nicknames:** “kiro”, “Kiro CLI”, “kiro agent”, “kiro subagent”
**Best default:** use as the company-provided coding harness and Codex
replacement in the corporate environment. It is suitable for general coding
work and also carries Jira/Confluence/GitHub MCP connectors.

Kiro runs one non-interactive CLI process per turn and cannot be steered while
a turn is in progress. Give it a complete, self-contained prompt. Use the model
hint as `"agent:model"`, `"agent:"`, or a bare model; for the company work agent,
for example, use `aumo-work:`.

Kiro CLI exposes no reasoning-effort control. Requires `kiro-cli` to be
installed and authenticated through its SSO login.
